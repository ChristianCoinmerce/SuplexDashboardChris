...
<div class="card">
  ...
  <div class="card-body">
    ...
    @foreach($latest_posts as $latest_post)
      <p>
        <strong><a href="{{ url('/'.$latest_post->slug) }}">{{ Str::limit($latest_post->title, 60) }}</a></strong>
  ...
  <div class="list-group-item" style="background-color: white;">Latest Comments</div>
    ...
    @foreach($latest_comments as $latest_comment)
      <div class="list-group-item" style="background-color: white">
        ...
        <div class="card-footer"></div>
        <p>On {{ $latest_comment->created_at->format('M d,Y \a\t h:i a') }}
        - On post <a href="{{ url('/'.$latest_comment->post->slug) }}">{{ $latest_comment->post->title }}</a></p>
      </div>
    @endforeach
    @else
