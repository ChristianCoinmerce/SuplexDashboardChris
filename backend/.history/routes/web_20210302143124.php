...
Route::group(['middleware' => 'myauthcheck:1'], function () {
    ...
    //POSTS CRUD
    Route::group(['middleware' => 'myauthcheck:7'], function () {
        Route::get('/dashboard/posts', [PostController::class, 'create_crud'])->name('posts.index');
    });
    Route::group(['middleware' => 'myauthcheck:6'], function () {
        Route::post('/dashboard/posts/store', [PostController::class, 'store_crud'])->name('posts.store');
    });
    Route::group(['middleware' => 'myauthcheck:8'], function () {
        Route::post('/dashboard/posts/update', [PostController::class, 'update'])->name('posts.update');
    });
    Route::group(['middleware' => 'myauthcheck:9'], function () {
        Route::get('/dashboard/posts/destroy/{id}', [PostController::class, 'destroy'])->name('posts.destroy');
    });

    //COMMENTS CRUD
    Route::group(['middleware' => 'myauthcheck:10'], function () {
        Route::get('/dashboard/comments', [CommentController::class, 'create'])->name('comment.index');
    });
    Route::group(['middleware' => 'myauthcheck:11'], function () {
        Route::post('/dashboard/roles/update', [CommentController::class, 'update'])->name('comment.update');
    });
    Route::group(['middleware' => 'myauthcheck:12'], function () {
        Route::get('/dashboard/comments/destroy/{id}', [CommentController::class, 'destroy'])->name('comment.destroy');
    });
